...
<summary>조평연</summary>
<div markdown="1">
<p>1.Call by value(값에 의한 호출)은 함수 호출 시 넘기는 인자의 값이 매개변수에 복사되는 방식입니다.</p>
<p>Call by value(값에 의한 호출)는 복사하여 처리하기 때문에 원래의 값이 보존되고 안전합니다.</p>
<p>그러나 복사를 하기 때문에 메모리 사용량이 늘어나는 단점이 있습니다.</p>
<br>
<p>2. Call by reference(참조에 의한 호출)는 인자로 받은 값의 주소를 복사하는 방식입니다.</p>
<p>Call by reference(참조에 의한 호출)는 복사하지 않고 직접 참조를 하기에 속도가 빠릅니다.</p>
<p>하지만 직접 참조를 하기에 원래 값이 영향을 받습니다.</p>
<br>
<p>C언어에서는 포인터를 이용해서 매개변수의 주소값을 넘겨 참조(Reference)할 수 있습니다.</p>
...
- 헥사고날(Hexagonal) 아키텍쳐에 대해서 설명해주세요.
<details>
<summary>계현준</summary>
<div markdown="1">
...
<summary>이동재</summary>
<div markdown="1">
<p>헥사고날 아키텍처는 도메인 중심 아키텍처 일종으로 클린 아키텍처를 일반화한 구조 중 하나입니다.</p>
<p>UI나 데이터베이스를 비즈니스 로직으로 분리되어야하는 외부요소로 취급합니다.</p>
<p>비즈니스 로직(도메인 코드)이 외부요소에 의존하지 않게 만들고, 프레젠테이션 계층(controller)과 데이터 소스 계층(persistence)이 도메인 계층에 의존하도록 합니다.</p>
<p>도메인 계층 입장에서 의존성이 대칭적입니다.</p>
<p>전체적으로 이 아키텍처는 비즈니스 로직을 다루는 내부와 기술적인 것을 다루는 외부로 나눌 수 있습니다.</p>
<p>외부에 포함된 컴포넌트를 어댑터라 부르고, 어댑터가 내부와 상호작용하는 부분을 포트라 부릅니다.</p>
<p>이 때문에 포트와 어댑터 아키텍처라고도 부릅니다.</p>
<li><계층 구성></li>
<p>헥사고날 아키텍쳐는 어댑터 계층, 애플리케이션 계층, 도메인 계층으로 나눌 수 있다.</p>
<p>1.어댑터 계층</p>
<p>어댑터 계층은 가장 바깥쪽에 있는 계층으로 애플리케이션과 다른 시스템간의 상호작용을 합니다.</p>
<p>웹 어댑터(=웹 컨트롤러), persistence 어댑터 등이 위치합니다.</p>
<p>2.애플리케이션 계층</p>
<p>애플리케션 계층은 바깥 계층에서 호출하기 위해 반드시 거쳐야 하는 곳입니다.</p>
<p>포트와 서비스를 포합합니다.</p>
<p>포트는 서비스에서 구현될 인터페이스로 input port와 output port가 있습니다.</p>
<p>서비시는 포트 구현체로 유스케이스 구현체라고도 합니다.</p>
<p>3.도메인 계층</p>
<p>도메인 계층은 도메인 엔티티들이 위치하고 있습니다.</p>
<li><패키지 구성></li>
<p>헥사고날 아키텍처를 spring 프로젝트에 적용하면 기존의 계층형 아키텍쳐에서 각 레이어별로 묶었기 때문에 모든 클래스들의 접근 제어자가 public으로 되어있어 경계가 약했던 단점을 보완할 수 있습니다.</p>
<p>1.어댑터 패키지는 package-private로 설정합니다.</p>
<p>2.애플리케이션 패키지는 어댑터에 접근해야할 포트는 public, 서비스는 package-private로 설정합니다.</p>
<p>input 포트를 public으로 지정했기 때문에 서비스는 오픈되지 않아도 됩니다.</p>
<p>3.도메인 패키지는 서비스, 어댑터를 접근해야하기 떄문에 public로 지정합니다.</p>
</div>
</details>
<details>
<summary>조평연</summary>
...
<summary>이동재</summary>
<div markdown="1">
<p>레어이드 아키텍처는 코드를 논리적인 부분 혹은 역할에 따라 독립된 모듈로 나누어서 구성하는 패턴입니다.</p>
<p>각 모듈이 서로의 의존도에 따라 층층히 쌓듯이 연결되어서 전체의 시스템을 구현하는 구조입니다.</p>
<p>마치 레이어를 쌓아 놓은 것 같은 형태의 구조가 됩니다.</p>
<p>레이어드 아키텍처의 핵심요소는 단방향 의존성입니다.</p>
...
<summary>이동재</summary>
<div markdown="1">
<p>웹에서 로그인을 처리하는 대표적인 방법으로는 세션 방식과 토큰 방식이 있습니다.</p>
<li><세선 방식></li>
<p>세션 방식은 서버의 메모리 or DB 등에 사용자의 정보를 저장하는 방식입니다.</p>
<p>서버는 사용자를 확인한 다음 세션 ID를 발급합니다.</p>
<p>토큰 방식에 비해 보안이 좋으나 서버의 자원이 상대적으로 많이 필요합니다.</p>
<li><토큰 방식></li>
<p>토큰 방식은 서버에서 발행하는 토큰을 사용자의 브라우저에 저장하는 방식입니다.</p>
<p>세션 방식에 비해 트레픽에 대한 부담이 적으나, 토큰이 유출될 경우 누구나 정보를 확인할 수 있어 보안이 상대적으로 좋지 않습니다.</p>
<p>또한 토큰에 담기는 데이터 크기가 길어지므로 잦은 요청 시 데이터 트레픽에 영향을 미칠 수 있습니다.</p>
</div>
</details>
...
<summary>조평연</summary>
<div markdown="1">
<p>REST(Representational State Transfer)API는 자원을 이름으로 구분하여 해당 자원의 상태를 주고받는 모든 것을 의미합니다</p>
<p>즉 REST란 HTTP URI(Uniform Resource Identifier)를 통해 자원(Resource)을 명시하는 것이고</p>
<p>HTTP Method(POST, GET, PUT, DELETE, PATCH 등)를 통해 해당 자원(URI)에 대한 CRUD Operation을 적용하는 것을 의미합니다</p>
</div>
</details>
...
<summary>조평연</summary>
<div markdown="1">
<p>프로세스가 단일 스레드로 동작하는 방식을 싱글스레드라고 합니다</p>
<p>두개 이상의 스레드가 프로세스 내부에서 자원을 공유하여 작업을 수행하는것을 멀티스레드라고 합니다</p>
</div>
</details>
...
<p>수평적 확장이란 더 많은 서버가 추가되고 데이터베이스가 전체적으로</p>
<p>분산되는것을 의미합니다</p>
</div>
</details>
...
<details>
  <summary>계현준</summary>
  <div markdown="1">
    전공자인 친구와 프로그래밍에 대해 이야기를 하며 '상상했던 기능을 실제로 구현해 낼 수 있다는 점'과 '한계 없는 가치를 만들어 낼 수 있다는 점'에 매력을 느껴 시작하게 되었습니다.
  </div>
</details>
<details>
<summary>이동재</summary>
...
<div markdown="1">
<p>영속성 컨텍스트는 엔티티를 영구 저장하는 환경을 의미합니다</p>
<p>애플리케이션과 데이터베이스 사이에서 객체를 보관하는 가상의 데이터베이스 역할을 합니다</p>
<p>엔티티 매니저를 통해 엔티티를 저장하거나 조회하면 엔티티 매니저는 영속성 컨텍스트에 엔티티를 보관하고 관리합니다</p>
</div>
</details> 
...
<summary>조평연</summary>
<div markdown="1">
<p>Spring framework (스프링 프레임워크)에서 front controller역할을 한다</p>
<p>실제 동작하기 위한 로직요청에 대해 처리할 수 있도록 해준다</p>
<p>dispatch의 뜻을 보내다라는 뜻으로</p>
<p>이곳에 요청이 오면 실제로 로직을 수행할 컴포넌트로</p>
<p>요청을 보내주는 역할을 하고 반환을 받는 역할을 한다</p>
<p>서비스의 예시를 들어보자면</p>
<p>서비스를 이용하는 유저가 웹서버에 원하는 요청을 보내면 처음에 디스패처 서블릿이라는 프론트 컨트롤러가 이 요청을 다받고</p>
<p>요청을 받은 디스패처 서블릿은 핸들러 매핑을 통해 이 요청에 맞는 컨트롤러를 결정한다</p>
<p>url이나 form data 등 여러개의 데이터 등을 기반으로 해당 컨트롤러를 결정하는 것이다</p>
</div>
</details> 

-   Spring Security의 구조에 대해 설명해주실 수 있을까요?
<details>
<summary>계현준</summary>
<div markdown="1">
...
<summary>이동재</summary>
<div markdown="1">
<p> Hibernate는 자바 언어를 위한 ORM 프레임워크입니다.</p>
<p> JPA의 구현체로, JPA 인터페이스를 구현하며, 내부적으로 JDBC API를 사용합니다.</p>
<p> Hibernate는 생산성과 유지보수면에서 장점이 있습니다.</p>
<p> Hibernate는 SQL을 직접 사용하지 않고, 메서드 호출만으로 쿼리가 수행됩니다.</p>
<p> 즉, SQL 반복 작업을 하지않음으로 생산성이 높아집니다.</p>
<p> 유지보수면에서는 테이블 컬럼이 변경되었을 때, 테이블과 관련된 DAO의 파라미터, 결과, SQL 등을 대신 수행해줍니다.</p>
<p> 이로 인해 유지보수면에서 큰 장점이 됩니다.</p>
<p> 그리고 특정 벤데에 종속적이지 않는데 설정 파일에서 JPA에게 어떤 DB를 사용하고 있는지를 알려주기만 하면 얼마든지 DB를 바꿀 수 있습니다.</p>
<p> Hibernate는 성능과 세밀함에서 단점이 있습니다.</p>
<p> 메서드 호출만으로 쿼리를 수행하는 것은 직접 SQL을 작성하는 것보다는 성능상 좋지 않습니다.</p>
<p> 또한 메서드 호출만으로 DB 데이터를 조작하기에는 한계가 있습니다.</p>
<p> 이를 보완하기 위해 JPQL을 지원합니다.</p>
</div>
</details>
...
<summary>이동재</summary>
<div markdown="1">
<p>스프링 프레임워크는 자바 개발을 편리하게 해주는 오픈소스 프레임워크입니다.</p>
<p>경량 컨테이너로서 자바의 객체 생성, 소멸과 같은 라이프 사이클을 관리해줍니다.</p>
</div>
</details>
...
<summary>이동재</summary>
<div markdown="1">
<p>가장 큰 차이점은 Auto Configuration의 차이인 것 같습니다. Spring은 프로젝트 초기에 다양한 환경설정을 해야 하지만,</p>
