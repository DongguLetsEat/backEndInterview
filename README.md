...
- JPA 연관관계 매핑 @OneToMany @ManyToOne @OneToOne @JoinColumn에 대해서 설명해주세요.
<details>
<summary>계현준</summary>
<div markdown="1">

- @OneToMany: 일대다(1:N) 관계를 매핑할 때 사용합니다. 예를 들어, 하나의 부서가 여러 명의 사원을 가질 수 있는 경우에 사용됩니다. 이 때, 부서 엔티티는 @OneToMany 어노테이션을 사용하여 사원 엔티티와 일대다 관계를 설정하며, 사원 엔티티는 @ManyToOne 어노테이션을 사용하여 부서 엔티티와 다대일 관계를 설정합니다.

- @ManyToOne: 다대일(N:1) 관계를 매핑할 때 사용합니다. 예를 들어, 여러 명의 사원이 하나의 부서에 소속될 수 있는 경우에 사용됩니다. 이 때, 사원 엔티티는 @ManyToOne 어노테이션을 사용하여 부서 엔티티와 다대일 관계를 설정합니다.

- @OneToOne: 일대일(1:1) 관계를 매핑할 때 사용합니다. 예를 들어, 하나의 사원이 하나의 주민등록번호를 가질 수 있는 경우에 사용됩니다. 이 때, 사원 엔티티와 주민등록번호 엔티티는 @OneToOne 어노테이션을 사용하여 일대일 관계를 설정합니다.

- @JoinColumn: 외래키를 매핑할 때 사용합니다. 예를 들어, 부서와 사원 엔티티 간에 일대다 관계를 설정할 때, 사원 엔티티에는 부서의 PK값을 저장할 FK(외래키)가 필요합니다. 이 때, @JoinColumn 어노테이션을 사용하여 FK와 관련된 정보를 매핑합니다.
